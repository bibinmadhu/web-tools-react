...
export const UnitTestModal: React.FC<UnitTestModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Modal Card */}
      <div className="relative z-50 w-full max-w-3xl bg-white dark:bg-[#1E293B] text-slate-900 dark:text-slate-100 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col max-h-[85vh]">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-[#0F172A]">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={isRunning}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-indigo-500/30 bg-indigo-50 dark:bg-indigo-500/10 hover:bg-indigo-100 dark:hover:bg-indigo-500/20 text-indigo-700 dark:text-indigo-300 text-xs font-semibold transition-colors disabled:opacity-50"
            >
        ...
        <div className="p-6 flex-1 overflow-y-auto space-y-5">
          {isRunning || !testSummary ? (
            <div className="py-16 text-center space-y-3">
              <RotateCw className="w-8 h-8 text-indigo-500 dark:text-indigo-400 animate-spin mx-auto" />
            ...
            <>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 text-center">
                  <div className="text-xs text-slate-500 dark:text-slate-400 font-mono">TOTAL TESTS</div>
                  <div className="text-xl font-bold text-slate-900 dark:text-white mt-0.5">
                    {testSummary.total}
                  </div>
                </div>
                <div className="p-3.5 rounded-xl bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 text-center">
                  <div className="text-xs text-emerald-600 dark:text-emerald-400 font-mono">PASSED</div>
                  <div className="text-xl font-bold text-emerald-600 dark:text-emerald-400 mt-0.5">
                    {testSummary.passed}
                  </div>
                </div>
                <div className="p-3.5 rounded-xl bg-rose-50 dark:bg-rose-500/10 border border-rose-200 dark:border-rose-500/20 text-center">
                  <div className="text-xs text-rose-600 dark:text-rose-400 font-mono">FAILED</div>
                  <div className="text-xl font-bold text-rose-600 dark:text-rose-400 mt-0.5">
                    {testSummary.failed}
                  </div>
                </div>
                <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 text-center">
                  <div className="text-xs text-slate-500 dark:text-slate-400 font-mono">DURATION</div>
              ...
              {/* Terminal Log View */}
              <div className="border border-slate-200 dark:border-slate-800 rounded-xl bg-slate-900 text-slate-200 p-4 font-mono text-xs space-y-2 shadow-inner">
                ...
                <div className="space-y-1.5 pt-2 max-h-60 overflow-y-auto">
                  {testSummary.results.map((r, i) => (
                    <div key={i} className="flex items-start justify-between py-1 border-b border-slate-800/80">
                      <div className="flex items-center gap-2">
                        {r.status === 'passed' ? (
        ...
        {/* Modal Footer */}
        <div className="px-6 py-3 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-[#0F172A] flex justify-end">
          <button
            onClick={onClose}
